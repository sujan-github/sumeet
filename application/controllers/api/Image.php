...
class Image extends REST_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->methods['image_get']['limit'] = 500; // 500 requests per hour per user/key
        $this->methods['image_post']['limit'] = 100; // 100 requests per hour per user/key
        $this->methods['image_delete']['limit'] = 50; // 50 requests per hour per user/key
    }
    ...
    public function image_get()
    {
        ...
        if ($id === null) {
            $imageArray = $this->image_model->get_all($query);

            // Check if the users data store contains users (in case the database result returns NULL)
            if ($imageArray) {
                // Set the response and exit
                $this->response($imageArray, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
            } else {
                // Set the response and exit
                $this->response([
                    'status' => false,
        ...
        $imageObject = $this->image_model->get_by_id($id, $query);

        if (!empty($imageObject)) {
            $this->set_response($imageObject, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
        } else {
            $this->set_response([
                'status' => false,
    ...
    public function image_post()
    {
        ...
        if ($this->post('Id')) {
            ...
            if ($this->image_model->put($this->post('Id'), $data)) {
                $message = [
